...
// import { getExtractedData } from '../../utils/formatting';
...
const configSlice = createSlice({
  ...
  reducers: {
    ...
    setExistingHistory(state, { payload }: PayloadAction<Array<SocketHistoryRecord>>) {
      // TODO when history is being replaced we have to extract the data
      // state.history = payload.map(item => ({ ...item, role: item.role, ...getExtractedData(item.content) }));
    ...
    // TODO from chat support
    // appendAssistantMessage(state, { payload: { content, id } }) {
    //   state.history.push({ ...getExtractedData(content), role: roles.assistant, id, time: new Date().getTime() });
    // },
    // appendUserMessage(state, { payload: { content, groupId, id } }) {
    //   state.history.push({ ...{ content, groupId }, role: roles.user, id, time: new Date().getTime() });
    // },
    // updateLastAssistantMessage(state, { payload }) {
    //   state.history = state.history.map((item, index) => {
    //     if (index === state.history.length - 1 && item.role === roles.assistant) {
    //       item = { ...item, content: item.content + payload };
    //     }
    //     return item;
    //   });
    // },
...
const fillMissingData = currentRecord => {
  if (currentRecord.role === Roles.assistant) {
    return {
      // @ts-ignore;
      ...currentRecord,
      content: currentRecord.content.map(record => {
        if (record.sequence) {
          return record;
        }
